...
class dataImporter {
    public:
        std::vector<std::pair<std::string, std::vector<std::string>>> importData(std::string path) {
            std::string line;
            ...
            if(!filestream.is_open()) throw std::runtime_error("Could not open file: " + path);
                ...
                while (std::getline(filestream, line)) {
                    ...
                    while (linestream >> column_headers[0] >> column_headers[1] >> column_headers[2] >> column_headers[3] >> column_headers[4] >> column_headers[5] >> column_headers[6]) {
                        if (!pairCreated) {
                            for (int i = 0; i < HEADER_SIZE; i ++) result.push_back({column_headers[i], std::vector<std::string> {}});
                            pairCreated = true;
